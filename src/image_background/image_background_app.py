...
with col1:
    ...
    if uploaded_file:
        ...
        st.image("src/image_background/images/example.jpg")
...
with col2:
    ...
    prompt_text = st.text_area(
        "Description including the object to keep and background to add:",
        value="Car at the beach",
        height=100,
        help="The prompt text",
    )

    negative_prompt = st.text_input(
        "What should not be in the background:", help="The negative prompt"
    )

    outpainting_mode = st.radio(
        "Outpainting mode:", ["DEFAULT", "PRECISE"], horizontal=True
    )
...
with col3:
    ...
    if generate_button:
        if uploaded_file:
            ...
            image_bytes = glib.get_bytes_from_file(
                "src/image_background/images/example.jpg"
            )
